defmodule S7 do
  ...
  `S7` is the public facade for connection lifecycle and PLC operations. A
  connected client is a PID whose `S7.Connection` process owns the TCP socket.
  Requests are queued and correlated by PDU reference, with concurrency bounded
  by Setup Communication negotiation.

  Protocol and transport modules are pure binary codecs and can be used
  independently for diagnostics and testing. S7comm-plus and symbolic
  addressing are outside the current scope.
  """
