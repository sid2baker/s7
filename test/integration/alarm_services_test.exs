defmodule S7.AlarmServicesIntegrationTest do
  ...
  test "subscribes, preserves ordered duplicates, queries, acknowledges, and tears down" do
    ...
    assert {:ok,
            %Alarm.Subscription{
             ...
             Client.subscribe_alarms(client, :alarm_8, queue_limit: 4)
    ...
    assert Client.acknowledge_alarm(client, first_object) == :ok
    ...
    assert {:error, %Error{reason: :invalid_subscription, operation: :next_alarm}} =
             Client.next_alarm(client, subscription, 10)
  ...
  test "supports ALARM_S subscriptions and empty alarm queries" do
    ...
    assert {:ok,
            %Alarm.Query{
              ...
              return_code: 0x0A,
              records: []
            }} = Client.query_alarms(client, :alarm_s)
  ...
  test "keeps a complete subscription rejection session-safe" do
    ...
    assert {:error, %Error{reason: :access_denied, code: 0xD241}} =
             Client.subscribe_alarms(client, :alarm_8)
  ...
  test "bounds unmatched and malformed indications without crashing" do
    for {fault, expected} <- [
      ...
      assert {:ok, client} = connect(server)
      assert {:ok, subscription} = Client.subscribe_alarms(client, :alarm_8)

      assert {:error, %Error{reason: ^expected}} =
               Client.next_alarm(client, subscription, 250)
  ...
  test "reports bounded queue overflow and still releases the remote subscription" do
    ...
    assert {:ok, client} = connect(server)
    assert {:ok, subscription} = Client.subscribe_alarms(client, :alarm_8, queue_limit: 1)

    Process.sleep(150)

    assert {:error, %Error{reason: :subscription_overflow, details: %{limit: 1}}} =
             Client.next_alarm(client, subscription, 100)
  ...
  test "invalidates rejected, missing, and malformed unsubscribe outcomes" do
    for {fault, expected} <- [
      ...
      assert {:error, %Error{reason: ^expected}} =
               Client.unsubscribe_alarms(client, subscription,
                 timeout: 100,
                 step_timeout: 500
               )
  ...
  test "returns per-item acknowledgment errors without invalidating the session" do
    ...
    assert {:error, %Error{reason: :pdu_too_large, details: %{outcome: :not_attempted}}} =
             Client.acknowledge_alarms(client, oversized)
  ...
  test "classifies complete and ambiguous acknowledgment failures" do
    ...
    assert {:error,
            ...
            }} = Client.acknowledge_alarm(rejected_client, acknowledgement(0xAF))

    assert %{state: :ready, exclusive_transaction: false} = Client.info(rejected_client)
    assert Client.close(rejected_client) == :ok
    ...
    for {fault, expected} <- [
      ...
      assert {:error, %Error{reason: ^expected, details: %{outcome: :indeterminate}}} =
               Client.acknowledge_alarm(client, acknowledgement(0xAF),
                 timeout: 100,
                 step_timeout: 500
               )
  ...
  test "closes the session when an alarm subscription owner dies" do
    ...
    owner =
      spawn(fn ->
        result = Client.subscribe_alarms(client, :alarm_8)
  ...
  test "validates alarm families, selectors, options, handles, and ownership locally" do
    ...
    for operation <- [
          fn -> Client.subscribe_alarms(client, :unknown) end,
          fn -> Client.subscribe_alarms(client, :alarm_8, queue_limit: 0) end,
          fn -> Client.subscribe_alarms(client, :alarm_8, subscription_key: "short") end,
          fn -> Client.subscribe_alarms(client, :alarm_8, unknown: true) end,
          fn -> Client.query_alarms(client, :scan) end
        ] do
      assert {:error, %Error{}} = operation.()
    end

    assert {:error, %Error{operation: :query_alarm}} = Client.query_alarm(client, -1)

    assert {:error,
            %Error{
              operation: :acknowledge_alarm,
              details: %{outcome: :not_attempted}
            }} = Client.acknowledge_alarm(client, :invalid)

    assert {:error,
            %Error{
              operation: :acknowledge_alarms,
              details: %{outcome: :not_attempted}
            }} = Client.acknowledge_alarms(client, [])

    refute_receive {:mock_plc_request, :alarm_subscribe, _reference}, 30

    assert {:ok, subscription} = Client.subscribe_alarms(client, :alarm_8)

    non_owner = Task.async(fn -> Client.next_alarm(client, subscription, 10) end)
    assert {:error, %Error{reason: :invalid_subscription}} = Task.await(non_owner)

    assert {:error, %Error{reason: :invalid_alarm_subscription}} =
             Client.unsubscribe_alarms(self(), subscription)
